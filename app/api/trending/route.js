...
function initializeFirebaseAdmin() {
  ...
  try {
    ...
    if (!adminSdkKey) {
      console.error('[Trending API] ❌ FIREBASE_ADMIN_SDK_KEY not set in environment');
      return null;
    }
    ...
    try {
      ...
      console.error('[Trending API] ❌ Failed to parse FIREBASE_ADMIN_SDK_KEY:', parseError.message);
      return null;
    }
    ...
    console.error('[Trending API] ❌ Error initializing Firebase Admin:', error.message);
    return null;
  }
}
...
export async function GET(request) {
  try {
    ...
    if (!db) {
      console.log('[Trending API] ⚠️ Firebase Admin not initialized');
      return NextResponse.json({
        success: true,
        products: [],
        connectedApis: [],
